...
export default class ImageCrop extends React.Component {
  ...
  componentDidMount() {
    ...
    $('body').addClass('bdy is_overlay-visible');
  }
  ...
  render() {
    ...
    const controlStyle = this.props.width < 300 ? { width: 300, marginTop: 30 } : { width: this.props.width };
    ...
    const ghostStyle = imageStyle;
    ghostStyle.backgroundImage = "url(" + this.state.element.src + ")";

    return (
      <div className="icr #{if this.state.visible then 'is_visible' else ''} #{if this.props.width < 400 then 'is_small'}">
